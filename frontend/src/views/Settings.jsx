  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('not a Tempa backup')
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="rocket" iconTint="var(--indigo)" title={t('Self-host Tempa')} subtitle={t('Passkey sign-in, sync across your devices, your own data.')} accessory="chevron"
    ...
    <Section title={t('General')} footer={t('Note: switching units only changes the label — logged numbers are not converted.')}>
      <SelectRow
        ...
        value={S.lang || deviceLang()} onChange={v => update(s => { s.lang = v })}
    ...
    <Section title={t('Data')}>
      <Row icon="sparkles" iconTint="var(--acc)" title={t('Build me a plan')}
        subtitle={t('Four questions — goal, days, equipment. Added to your plan, nothing removed.')}
        accessory="chevron" onClick={() => nav('/plan/new')} />
      <Row icon="clipboard" iconTint="var(--acc)" title={t('Load starter plan (PPL)')} accessory="chevron" onClick={loadStarterPlan} />
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install Tempa as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}

    {/* Attribution, not a credits roll. The AGPL requires the notices to travel
        with the work, and a fork that quietly drops the name of the project it
        is built on is doing something worse than a licence breach. Both links
        stay reachable from inside the app, not only from the repository. */}
    <div className="dim small" style={{ textAlign: 'center', marginTop: 4, lineHeight: 1.7 }}>
      Tempa v{APP_VERSION} · {t('free & open source (AGPL v3)')}<br />
      {t('A fork of openGym by Duarte Santos.')}<br />
      <a href="https://github.com/fareza777/opengym-claude" target="_blank" rel="noopener">{t('Source code')}</a>
      {' · '}
      <a href="https://github.com/DuarteSantos8/openGym" target="_blank" rel="noopener">openGym</a>
      {' · '}
      <a href="https://github.com/hasaneyldrm/exercises-dataset" target="_blank" rel="noopener">{t('Exercise data')}</a>
    </div>
  </div>
}
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if Tempa is closed.')}>
