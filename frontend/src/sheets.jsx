...
function PlanTools({ close }) {
  ...
  return <>
    ...
    <div className="dim small" style={{ margin: '7px 2px 0', lineHeight: 1.4 }}>{t('A small file a friend imports into their own Tempa — routines only, none of your workouts or weigh-ins.')}</div>
    {!MOBILE && <>
      <div style={{ height: 12 }} />
...
function TopWeight({ entryIdx, close }) {
  ...
  const unitDone = !!entry && unit.every(i => A.entries[i].sets.every(s => s.done || s.warm))
...
function doFinishWorkout() {
  ...
  const w = {
    ...
    entries: A.entries.map(e => ({ id: e.id, sets: e.sets, topW: e.topW || null, target: e.target || null })).filter(e => e.sets.some(s => s.done && !s.warm)),
    prs
  }
